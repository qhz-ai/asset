<template>
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :disabled="form.disable" :rules="rules" label-width="80px">
        <el-form-item label="标题" prop="title">
        ...
        <el-form-item label="链接" prop="url">
          <el-input v-model="form.url" placeholder="请输入链接" v-if="!form.disable"/>
          <span v-if="form.disable && form.url != null">
            <router-link :to="form.url" class="link-type">
              <span>打开</span>
            </router-link>
          </span>
        </el-form-item>
...
<script>
import { inMsg, getMsg, delMsg, addMsg, updateMsg, readMsg } from "@/api/sys/msg";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      inMsg(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleRead(row) {
      const ids = row.id || this.ids;
      this.$confirm('是否确认将选中的消息标记为已读?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
